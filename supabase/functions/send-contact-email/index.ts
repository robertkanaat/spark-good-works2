...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const isBookDownload = subject.includes('Book Download Request');
    const confirmationSubject = isBookDownload ? 
      "Your Addiction Recovery Book is Ready for Download!" : 
      "Thank you for contacting Genius Recovery";
    
    const confirmationHtml = isBookDownload ? `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        <meta charset="UTF-8">
